...
Metashape 2.x renamed several workflow steps. The instructions use the current names, with the older name in parentheses the first time it appears, so that older tutorials and videos still make sense.
...
Click Optimize Cameras ![Optimize icon](images/optimize_icon.webp){width=25} in the Reference pane toolbar and check all the camera parameters.
...
Click Settings ![Settings icon](images/settings_icon.webp){width=25} and leave the default marker accuracy (0.005 m).
...
1. Optimize Cameras (only if the GCPs were placed but the alignment not yet optimized)
2. Build Point Cloud
3. Build Model
4. Build Texture
5. Build DEM
...
* **Report interpretation**: the accuracy figures are identified correctly and explained, not just copied; control error and checkpoint error are distinguished.
