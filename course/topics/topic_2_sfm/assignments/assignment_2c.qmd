...
- Two files prepared for WebODM, explained below:
  ...
  - [gcp_seed_epsg32617.txt](assets/gcp_seed_epsg32617.txt): the three GCP coordinates in WGS84 UTM 17N, in the layout the GCP interface reads (also available in latitude and longitude as [gcp_seed_epsg4326.txt](assets/gcp_seed_epsg4326.txt))
...
2. Load the seed file `gcp_seed_epsg32617.txt`: it lists the three GCPs with their coordinates and no image positions yet. ODM recommends a projected system for ground control; `gcp_seed_epsg4326.txt` holds the same points in latitude and longitude if you need them.
...
* **Comparison**: the two tools are compared with figures and numbers.
