...
```bash
i.superpixels.slic input=vis_bands output=superpixels num_pixels=10000
...
To train the model you will use [r.learn.train](https://grass.osgeo.org/grass-stable/manuals/addons/r.learn.train.html). The random forest model requires us to provide a [training_map](resources/training.gpkg) and our imagery group. To save time use our `rgb_group`, but in practice you should using the `odm_071724_ortho` group that contains the additional features you created.
...
To use the model run [r.learn.predict](https://grass.osgeo.org/grass-stable/manuals/addons/r.learn.predict.html) by loading the model (`rf_model.gz`) you create during training.
