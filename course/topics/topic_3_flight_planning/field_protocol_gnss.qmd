...
Read this before the {{< var course_params.term >}} Lake Wheeler field flight. The site is available from 10:15 to 11:30. The class flies the block with a DJI Mavic Air 2 and surveys the ground control and checkpoint coordinates for the Topic 3B lab. You will be on the flight crew, with the instructor on the aircraft, or the GNSS crew, rotating through the Emlid Reach RS4 rover off the flight line.

::: {.callout-note}
**Distance education students**: read this protocol, then watch [How to Place Ground Control Points (GCPs) for Precise Drone Mapping](https://www.youtube.com/watch?v=uWDz1NpWGtA) for the target placement and survey in sections 3 and 4.
:::

Two things to keep in mind. The Mavic Air 2 geotags its photos to within a few meters; the ground targets bring the flight to centimeters, so they go down first. The rover gets its fix from the North Carolina Real Time Network (NCRTN) over the tablet's cell connection, not from the base. The base is a logger on the mark for the Topic 3B lab, and nobody touches it once it is running.
...
2. The instructor gives an overview: what is being flown, why the targets and checkpoints exist, who is on which crew, and where not to stand.
3. Flight crew lays the four targets. GNSS crew does Method B on the mark (section 1).
4. Flight crew preflights the aircraft. GNSS crew sets up the base and starts the log (section 2).
5. Launch. One battery over the target field (section 3).
6. While the aircraft flies, the GNSS crew surveys the four targets, then 8 to 10 checkpoints (section 4).
7. Land. Flight crew checks the photo count and geotags. A second GNSS group re-occupies 3 checkpoints.
8. Stop the rover and export the project. Stop the base log last. Pack up (section 5).
...
The instructor is the Remote Pilot in Command. Flight crew students act as visual observers and may take the controls under supervision.
...
- Run the preflight checklist. No airspace authorization is needed: Lake Wheeler is under the RDU Class C shelf and the flight stays at or below 400 ft AGL.
...
Project settings: NAD83(2011) / North Carolina in meters (EPSG:6542) with NAVD88 (GEOID18) height. The labs use EPSG:3358, NAD83(HARN), which differs by about 2 cm here, so write the datum on the field sheet.

**Targets first (4 points).** Measure the center of each with a 30 second averaged FIX. These are the only points used to control the processing.

**Checkpoints (8 to 10 points).** Withheld from processing; they measure the accuracy of the products.
...
- Mavic Air 2: two batteries charged, the block planned for one battery, B4UFLY and the NCRTN status page checked the morning of
