...
The base station sits on the practice mark at 35.727565, -78.697788. The Topic 3B lab establishes the coordinate of that mark three ways and compares them:

- **Method A**: the base receiver's own averaged position, with no corrections. Expect meters of error.
- **Method B**: a real-time network (RTK) fix on the mark, collected with the rover using corrections from the North Carolina Real Time Network (NCRTN). Expect a few centimeters.
...
- Fly one battery, one block over the GCP field. The second battery is the spare.
